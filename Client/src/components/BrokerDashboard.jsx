...
import { Layout, Card, Row, Col, Typography, Space, Statistic, Table, Tag, List, Avatar, Breadcrumb, Button, Input } from 'antd';
import {
  UserOutlined, ShoppingCartOutlined, DollarOutlined, RiseOutlined,
  SyncOutlined, HomeOutlined, DashboardOutlined, MailOutlined, DownloadOutlined
...
const BrokerDashboard = () => {
  ...
  return (
    <Layout style={{ minHeight: '100vh' }}>
      <Dashboardsidebar2 collapsed={collapsed} />
      <Layout>
        <DashboardNavbar />
        <Content style={{ margin: '24px 16px', padding: 24, background: '#fff' }}>
          <Breadcrumb style={{ marginBottom: 24 }} items={[{ title: 'Dashboard' }]} />
          <Title level={4}>Broker Dashboard</Title>
          ...
          <Row gutter={[24, 24]} style={{ marginTop: 24 }}>
            <Col xs={24} md={12}>
              <Card title="Top Selling Products" extra={<Button type="primary" icon={<DownloadOutlined />}>Export</Button>}>
                <Table
                  columns={columns}
            ...
            <Col xs={24} md={12}>
              <Card title="Recent Activity" extra={<Button type="primary" icon={<SyncOutlined />}>Refresh</Button>}>
                <List
        ...
        <Footer style={{ textAlign: 'center' }}>
          Promoter Property ©2023 Created by Promoter Property
        </Footer>
      </Layout>
    </Layout>
  );
